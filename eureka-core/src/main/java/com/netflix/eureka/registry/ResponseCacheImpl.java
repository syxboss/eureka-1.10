...
public class ResponseCacheImpl implements ResponseCache {
    ...
    ResponseCacheImpl(EurekaServerConfig serverConfig, ServerCodecs serverCodecs, AbstractInstanceRegistry registry) {
        ...
        this.readWriteCacheMap =
                CacheBuilder.newBuilder().initialCapacity(serverConfig.getInitialCapacityOfResponseCache())
                        // 设置过期时间  180s
                        // readWriteCacheMap放入数据后，会等180s后，数据设置为过期（定时过期）
    ...
    // 每隔30s，会对只读缓存和读写缓存中的数据进行一个比对，如果两块数是不一致的，那么就将读写缓存的数据放到只读缓存中
     ...
     */
    @VisibleForTesting
    Value getValue(final Key key, boolean useReadOnlyCache) { // 双map机制缓存
        Value payload = null;
        try {
            if (useReadOnlyCache) {
                //readOnlyCacheMap只读缓存
                final Value currentPayload = readOnlyCacheMap.get(key);
                if (currentPayload != null) {
                    payload = currentPayload;
                } else {
                    //readWriteCacheMap读写缓存
                    //将注册表读取的applications放到读写缓存，，并放到只读缓存
    ...
    private String getPayLoad(Key key, Applications apps) {
        // serverCodecs 进行json序列化的组件
        // 将application对象序列化json
